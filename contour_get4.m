...
if (sum(sum(BW))<10),                                                       % we consider that less than 3 pixels cannot make a contour
    C=[0 0]; 
    after=BW;
    return; 
end;
...
%MAJ=[6 6 0 0 2 2 4 4];%[0 0 2 2 4 4 6 6];%                                                     % variable initialization
C=[row,col;0,0];
...
while(ended==0),
    ...
    while(found_next==0),
        ...
        if (found_next==0&&w<8)
            ...
            direction=direction+(-1)^w*w;% w是当前点已经判断的次数
