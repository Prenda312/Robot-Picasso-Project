...
A=imread(['C:\Users\Lenovo\Desktop\机器人建模Project\生成单字库\',dataSet(cla),num2str(fea),'.png']);
figure(1),imshow(A) %显示读取的手写灰度图'C:\Users\Lenovo\Desktop\工程文件\改进后工程文件--贝叶斯分类器\字库\生成单字库\',code(j),num2str(num),'.png'
B=zeros(1,400);     %创建1行100列的数组
pattern(cla).feature(:,fea)=zeros(400,1); %初始化当前类的第fea个初始化特征向量
[row,col] = size(A); %获取样本图片的行列
cellRow = row/20;    %除以10得到1/100的小个子
cellCol = col/20;
...
pattern(cla).num=8;             %类的特征数量(feature中的列数)
pattern(cla).name=dataSet(cla);
%当前类代表的汉字字符
